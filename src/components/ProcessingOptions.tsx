...
export default function ProcessingOptions({ options, onChange }: ProcessingOptionsProps) {
  return (
    <div className="flex flex-col gap-4 md:flex-row">
      ...
      <div className="glassmorphism-card glassmorphism-card--orange flex-1 rounded-2xl px-5 py-5 space-y-3">
        <Label className="text-[10px] text-muted-foreground/60 font-mono tracking-[0.15em] uppercase block">Output Format</Label>
        <div className="flex w-full rounded-xl bg-background/30 p-1 gap-0.5">
          {FORMAT_OPTIONS.map((opt) => (
            <button
              key={opt.value}
              onClick={() => onChange({ ...options, outputFormat: opt.value as Options["outputFormat"] })}
              className={`
                flex-1 text-[11px] font-medium py-1.5 px-2 rounded-lg transition-all duration-300 whitespace-nowrap
      ...
      <div className="glassmorphism-card glassmorphism-card--purple flex-1 rounded-2xl px-5 py-5 space-y-3">
